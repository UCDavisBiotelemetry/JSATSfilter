# Version MP_TeknoFilter2.5.1.2_20180321.R
###################################################################################################################
#
...
#                                   Last Updated: 03/21/2018 by Matt Pagel
#                                           "Version" 2.5.1.2
#
...
cleanInnerWrap <-function(...) {
  itercount <- 0
  function(i, tags, headerInFile, leadingBlanks, tz="GMT", foutPrefix, inferredHeader, Rec_dtf_Hex_strings=c("ReceiverID","DetectionDate","TagID")) {
    if (!headerInFile) {
    ...
    setDT(dat, key=c("Hex")) # convert to data.table # key=c("RecSN","Hex","dtf"))
    dat[,dtf:=as.POSIXct(dtf, format = "%Y-%m-%d %H:%M:%OS", tz=tz)]
    setkey(tags,TagID_Hex)
    dat2<-dat[tags,nomatch=0] # bring in the nominal PRI (nPRI)
    # combine the DT and FracSec columns into a single time column and convert to POSIXct
    setkey(dat2, RecSN, Hex, dtf)
