...
if (DoCleanSUM) {
  for(i in list.files("./raw/*.SUM", full.names=TRUE)){
...
if (DoCleanATS) {
  for(i in list.files("./raw/*.XLS*", full.names=TRUE)){
    if (as.integer(file.info(i)["isdir"])) next
    cleanATS(i, tags, max(counter)) 
  }
}

if (DoCleanLotek) {
  for(i in list.files("./raw/*.TXT", full.names=TRUE)){
    if (as.integer(file.info(i)["isdir"])) next
    cleanLotek(i, tags, max(counter))
  }
}
