...
#                                   Last Updated: 11/31/2017 by Matt Pagel                                                         #
#                                                                                                                                  #
####################################################################################################################################
...
TAGFILENAME = "./taglist/Brandes1.csv"
DoCleanJST = FALSE
DoCleanSUM = TRUE
DoCleanATS = FALSE
DoCleanLotek = FALSE
      ...
      if (file == "") {
        ...
        if (keep.source) {
          ...
          if (!length(text)) {
            ...
            text <- gsub("(, .internal.selfref = <pointer: 0x[0-9A-F]+>)","",text,perl=TRUE)
          }
          on.exit(close(file))
        }
      }
...
#  LT<-WindHits[,isAccepted:=!is.na(intervals)][,c:=length(.SD[!is.na(intervals)],keyby="firstHit")]
#  setkey(noNAs,hit)
...
#  wh<-unique(noNAs[,hit])
#  setkey(noNAs,firstHit,intervals)
